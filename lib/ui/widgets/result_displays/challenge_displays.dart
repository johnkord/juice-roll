...
void registerChallengeDisplays() {
  ResultDisplayRegistry.register<FullChallengeResult>(
      _buildFullChallengeDisplay);
  ResultDisplayRegistry.register<ChallengeSkillResult>(
      _buildChallengeSkillDisplay);
...
Widget _buildFullChallengeDisplay(FullChallengeResult result, ThemeData theme) {
  return Column(
    ...
    children: [
      Row(children: [
        _buildChallengeOptionChip(result.physicalSkill, result.physicalDc,
            result.physicalRoll, JuiceTheme.danger, theme),
        const SizedBox(width: 12),
        _buildChallengeOptionChip(result.mentalSkill, result.mentalDc,
            result.mentalRoll, JuiceTheme.info, theme),
      ]),
      const SizedBox(height: 6),
      Text('(${result.dcMethod})',
...
Widget _buildChallengeOptionChip(
    String skill, int dc, int roll, Color color, ThemeData theme) {
  return Container(
...
Widget _buildChallengeSkillDisplay(
    ChallengeSkillResult result, ThemeData theme) {
  final color = result.challengeType == ChallengeType.physical
      ? JuiceTheme.danger
      : JuiceTheme.info;
  return _buildChallengeChip(
      result.challengeType.name, result.skill, result.roll, color, theme);
}

Widget _buildChallengeChip(
    String label, String skill, int roll, Color color, ThemeData theme) {
  return Container(
...
Widget _buildDcDisplay(DcResult result, ThemeData theme) {
  return Row(children: [
    Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(
          color: JuiceTheme.gold15, borderRadius: BorderRadius.circular(6)),
      child: Text(result.diceResults.join(", "),
          style: TextStyle(
              fontFamily: JuiceTheme.fontFamilyMono,
              fontSize: 11,
              color: JuiceTheme.gold)),
    ...
    Text('DC ${result.dc}',
        style: theme.textTheme.titleMedium
            ?.copyWith(fontWeight: FontWeight.bold, color: JuiceTheme.gold)),
    const SizedBox(width: 8),
    Text('(${result.method})',
        style: theme.textTheme.bodySmall?.copyWith(color: Colors.grey)),
  ]);
...
Widget _buildQuickDcDisplay(QuickDcResult result, ThemeData theme) {
  return Row(children: [
    Container(
      ...
      decoration: BoxDecoration(
          color: JuiceTheme.gold15, borderRadius: BorderRadius.circular(6)),
      child: Text('2d6: [${result.dice.join(", ")}]',
          style: TextStyle(
              fontFamily: JuiceTheme.fontFamilyMono,
              fontSize: 11,
              color: JuiceTheme.gold)),
